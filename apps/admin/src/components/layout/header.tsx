...
interface AdminNotificationType {
    _id: string;
    event: string;
    payload?: { description?: string;[key: string]: any };
    read?: boolean;
    isRead?: boolean;
    createdAt: string;
}
...
export function Header() {
    ...
    useEffect(() => {
        ...
        // Reset unread count when opening notifications or when new ones arrive
        // For simplicity with useQuery, we'll keep unreadCount logic basic
    ...
    return (
        <header className="flex h-[72px] items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-8 shrink-0 z-10 shadow-[0_4px_24px_-12px_rgba(0,0,0,0.05)]">
            ...
            <div className="flex items-center gap-2 lg:gap-3 ml-auto">
                {/* Live Clock */}
                {currentTime && (
                    <div className="hidden md:flex flex-col items-end mr-4 pr-4 border-r border-slate-200 dark:border-slate-800">
                ...
                <div className="relative" ref={notifRef}>
                    <Button
                        variant="ghost"
                        size="icon"
                        className="relative text-muted-foreground/70 hover:text-foreground rounded-full hover:bg-muted"
                    ...
                    {isNotifOpen && (
                        <div className="absolute right-0 md:right-auto md:-left-32 mt-2 w-[340px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg flex flex-col z-50 overflow-hidden transform-gpu origin-top-right">
                            <div className="px-4 py-3 border-b border-border/50 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-700/50">
                                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">Change Log</h3>
                                {unreadCount > 0 && (
                ...
                <Button
                    variant="ghost"
                    size="icon"
                    className="text-muted-foreground/70 hover:text-foreground rounded-full hover:bg-muted"
                    onClick={() => {
