...
export async function fetchDailyAnalytics(date: string) {
    const res = await get(`/analytics/daily/${date}`);
    if (!res?.success) throw new Error("Failed to fetch daily analytics");
    return res.data;
}


export async function fetchWeeklyAnalytics(week: string) {
    const res = await get(`/analytics/weekly/${week}`);
    if (!res) return null;
    if (!res.success) throw new Error("Failed to fetch weekly analytics");
    return res.data;
}

export async function fetchMonthlyAnalytics(month: string) {
    const res = await get(`/analytics/monthly/${month}`);
    if (!res) return null;
...
export async function fetchAllTimeAnalytics() {
    const res = await get(`/analytics/all-time`);
