...
import { fetchAllTimeAnalytics, fetchDailyAnalytics, fetchWeeklyAnalytics, fetchMonthlyAnalytics, get } from "@/lib/api"
...
export default function DashboardPage() {
    ...
    // Section 2: Core Analytics Snapshots (Strict Snapshot Mode)
    const { data: dailyData, isLoading: isLoadingDaily, isFetching: isDailyFetching } = useQuery({
        queryKey: ["daily-snapshot-dash"],
        queryFn: () => {
            const today = new Date().toISOString().split('T')[0]
            return fetchDailyAnalytics(today)
        },
        retry: false
    }) as { data: any, isLoading: boolean, isFetching: boolean }

    const { data: weeklyData, isLoading: isLoadingWeekly, isFetching: isWeeklyFetching } = useQuery({
        ...
        queryFn: () => {
            ...
            const weekStr = `${d.getUTCFullYear()}-${String(weekNo).padStart(2, '0')}`
            return fetchWeeklyAnalytics(weekStr)
        },
        enabled: isChartExpanded && chartTab === 'weekly',
        retry: false
    }) as { data: any, isLoading: boolean, isFetching: boolean }

    const { data: monthlyData, isLoading: isLoadingMonthly, isFetching: isMonthlyFetching } = useQuery({
        queryKey: ["monthly-snapshot-dash"],
        queryFn: () => {
            const monthStr = new Date().toISOString().slice(0, 7)
            return fetchMonthlyAnalytics(monthStr)
        },
        enabled: isChartExpanded && chartTab === 'monthly',
        retry: false
    }) as { data: any, isLoading: boolean, isFetching: boolean }

    // Section 3: Operational Insights (Complaints & Reviews)
    const { data: complaintsData, isLoading: isLoadingComplaints } = useQuery({
        queryKey: ["complaints-snapshot"],
        queryFn: () => get("/complaints/admin/list?limit=100"),
        retry: false
    }) as { data: any, isLoading: boolean }

    const { data: reviewsData, isLoading: isLoadingReviews } = useQuery({
        queryKey: ["reviews-snapshot"],
        queryFn: () => get("/admin/reviews/flagged?limit=100"),
        retry: false
    }) as { data: any, isLoading: boolean }
    ...
    const complaintDistribution = useMemo(() => {
        ...
        complaintsData.data.forEach((c: { status: string }) => {
            if (counts[c.status as keyof typeof counts] !== undefined) {
                counts[c.status as keyof typeof counts]++
            }
        })
        return [
            { name: 'Open', value: counts.OPEN, color: COLORS.danger },
            { name: 'In Progress', value: counts.IN_PROGRESS, color: COLORS.warning },
            { name: 'Resolved', value: counts.RESOLVED, color: COLORS.success },
            { name: 'Rejected', value: counts.REJECTED, color: COLORS.border }
        ].filter(d => d.value > 0)
    }, [complaintsData])
    ...
    const reviewBreakdown = useMemo(() => {
        ...
        reviewsData.data.forEach((r: { status: string }) => {
            if (counts[r.status as keyof typeof counts] !== undefined) {
                counts[r.status as keyof typeof counts]++
            }
        })
        return [
            { name: 'Pending', value: counts.OPEN, color: COLORS.warning },
            { name: 'Resolved', value: counts.RESOLVED, color: COLORS.success },
            { name: 'Rejected', value: counts.REJECTED, color: COLORS.border }
        ]
    }, [reviewsData])

    const activeSnapshot = chartTab === 'daily' ? dailyData : chartTab === 'weekly' ? weeklyData : monthlyData
    const isChartLoading = chartTab === 'daily' ? (isLoadingDaily || isDailyFetching)
        : chartTab === 'weekly' ? (isLoadingWeekly || isWeeklyFetching)
            : (isLoadingMonthly || isMonthlyFetching)
        ...
        <PageContainer>
            ...
            <GridSystem cols={5}>
                <KPICard
                    ...
                    status={summary?.totalProviders > 0 ? "Registered on platform" : "No providers yet"}
                ...
                <KPICard
                    ...
                    status={summary?.totalReviews > 0 ? "Reviews logged" : "Looking good"}
            ...
            <GridSystem cols={2}>
                ...
                <Card>
                    <CardHeader className="pb-2 space-y-0">
                        <CardTitle className="text-base">Review Moderation</CardTitle>
                        <p className="text-xs text-slate-500 mt-1">Status breakdown for flagged reviews</p>
                    </CardHeader>
                    <CardContent>
                        <div className="h-[220px]">
                            {isLoadingReviews ? (
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={reviewBreakdown} margin={{ top: 10, right: 20, left: 40, bottom: 10 }} layout="vertical">
                                        <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#E2E8F0" />
                                        <XAxis type="number" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} />
                                        <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#0f172a', fontWeight: 500 }} width={100} />
                                        <Tooltip
                                            cursor={{ fill: '#f8fafc' }}
                                            contentStyle={{ borderRadius: '8px', border: '1px solid #E2E8F0', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', fontSize: '13px' }}
                                        />
                                        <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={24}>
                                            {reviewBreakdown.map((entry, index) => (
                                                <Cell key={`cell-${index}`} fill={entry.color} />
                                            ))}
                                        </Bar>
                                    </BarChart>
...
function IntentSummarySection() {
    const { data: intentRes, isLoading, error } = useQuery({
        queryKey: ["session-intent-summary"],
        queryFn: () => get<any>("/analytics/conversion/session-based"),
        retry: false,
        refetchOnWindowFocus: false
    })
    ...
    const abandonment = data?.intentConcerns?.BOOKING_ABANDONED || 0
    const totalWithSearch = summary?.totalSessionsWithSearch || 0
    ...
    return (
        <div>
            ...
            <GridSystem cols={2}>
                ...
                <IntentCard
                    title="Abandonment"
                    value={`${(abandonment / (totalWithSearch || 1) * 100).toFixed(1)}%`}
                    isLoading={isLoading}
                />
            </GridSystem>
        </div>
...
function ComplaintResolutionSection() {
    const { data: weeklySnapshot, isLoading } = useQuery({
        queryKey: ["weekly-snapshot-complaints"],
        queryFn: () => {
            const d = new Date()
            d.setUTCDate(d.getUTCDate() + 4 - (d.getUTCDay() || 7))
            const yearStart = new Date(Date.UTC(d.getUTCFullYear(), 0, 1))
...
function ProviderStatusSummary({ data, isLoading }: { data: any, isLoading: boolean }) {
    if (isLoading) return (
    ...
    return (
        <Card className="overflow-hidden border-slate-200">
            <CardHeader className="pb-3">
                <CardTitle className="text-sm font-bold uppercase tracking-wider text-slate-600">Provider Status Summary</CardTitle>
            </CardHeader>
    ...
    return (
        <div className="flex items-center justify-between text-sm group">
            <div className="flex items-center gap-2">
                ...
                <span className="text-slate-500 font-medium group-hover:text-slate-900 transition-colors">{label}</span>
            </div>
            <span className="font-bold text-slate-900">{formatNum(count)}</span>
        </div>
    )
}
    ...
    return (
        <Card className="bg-white border-slate-200">
            <CardContent className="p-4 flex flex-col pt-4">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-tight mb-1">{title}</span>
                <span className="text-2xl font-black text-slate-900">
                    {isLoading ? <div className="h-8 w-16 bg-slate-100 animate-pulse rounded" /> : value}
                </span>
            </CardContent>
        </Card>
    )
...
function KPICard({ title, value, status, onClick }: { title: string, value: string | number, status: string, onClick: () => void }) {
    return (
        <Card onClick={onClick} className="group cursor-pointer hover:border-violet-500 transition-all hover:shadow-[0_4px_12px_rgba(79,70,229,0.08)]">
            <CardContent className="p-6 pt-6">
                <span className="text-xs font-medium text-slate-500 mb-2 uppercase tracking-wider block">{title}</span>
                <span className="text-3xl font-bold text-slate-900 mb-3 block">{value}</span>
                {status && (
                    <span className="text-sm text-violet-600 font-medium block">{status}</span>
                )}
            </CardContent>
        </Card>
    )
