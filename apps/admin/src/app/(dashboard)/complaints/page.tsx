...
import { get, patch, logAdminAction } from "@/lib/api"
...
type TabType = "active" | "resolved" | "rejected"
    ...
    const loadComplaints = async (currentPage = page, tab = activeTab) => {
        ...
        try {
            ...
            if (tab === "active") {
                ...
                endpoint = `/admin/complaints?status=REJECTED&page=${currentPage}&limit=${limit}`
            }
    ...
    return (
        <PageContainer className="flex flex-col h-[calc(100vh-100px)] !space-y-6">
            <SectionHeader
                title="Complaints"
                action={
                    <Tabs value={activeTab} onValueChange={(v) => handleTabChange(v as TabType)} className="w-[340px]">
                        <TabsList className="grid w-full grid-cols-3 bg-slate-100 p-1 rounded-lg">
                            ...
                            <TabsTrigger value="rejected" className="rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm">Rejected</TabsTrigger>
                        </TabsList>
                    </Tabs>
                }
            />
