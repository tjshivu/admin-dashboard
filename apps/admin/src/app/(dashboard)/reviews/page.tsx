...
export default function ReviewsPage() {
    ...
    const resolveReport = async (status: "RESOLVED" | "REJECTED") => {
        ...
        if (!adminNotes) {
            showToast({ type: "warning", message: "Admin note is required to resolve or reject a report." })
            return
        }
        setActionLoading(true)
        try {
            await put(`/admin/reviews/reports/${selectedReport._id}/resolve`, {
                status,
                admin_notes: adminNotes
            })
            // Remove from local flagged list since status is no longer OPEN
            setReports(prev => prev.filter(r => r._id !== selectedReport._id))
            setSelectedReport(null)
            setSelectedReview(null)
            showToast({ type: "success", message: `Report mathematically closed as ${status}` })
        } catch (e) {
            console.error(e)
            showToast({ type: "error", message: "Failed to resolve report" })
        } finally {
            setActionLoading(false)
        }
    }
    ...
    return (
        <PageContainer className="flex flex-col h-[calc(100vh-100px)] !space-y-6">
                ...
                action={
                    <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as "all" | "flagged" | "ai_flagged")} className="w-[400px]">
                        <TabsList className="grid w-full grid-cols-3 bg-slate-100 p-1 rounded-lg">
                            ...
                            <TabsTrigger value="flagged" className="rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm">Reports</TabsTrigger>
                            <TabsTrigger value="ai_flagged" className="rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm">AI Flagged</TabsTrigger>
                        </TabsList>
                    </Tabs>
                }
            />
            ...
            <Card className="flex-1 overflow-hidden flex flex-col lg:flex-row p-0 min-h-0 bg-white shadow-sm border-slate-200">
                {/* Left Panel: List */}
                <div className="w-full lg:w-[450px] flex flex-col border-b lg:border-b-0 lg:border-r border-slate-200 shrink-0 bg-slate-50">
                    <div className="p-4 border-b border-slate-200 bg-slate-50/50 font-semibold text-sm text-slate-800 shrink-0 shadow-sm z-10">
                            ...
                            (() => {
                                ...
                                return displayReviews.map(r => (
                                    <button
                                        ...
                                        className={cn(
                                            "w-full text-left p-4 border-b border-slate-100 transition-all hover:bg-slate-100/50 flex flex-col gap-2",
                                            selectedReview?._id === r._id && "bg-violet-50 hover:bg-violet-50 border-l-2 border-l-violet-600 border-b-violet-100"
                                        )}
                                    >
                                        <div className="flex justify-between items-start w-full">
                                            <div className="font-semibold text-sm text-slate-900 truncate pr-2">{r.user_id?.name || 'Unknown User'}</div>
                                            <div className="flex items-center gap-1 shrink-0 text-amber-500 font-bold text-xs">
                                                {r.average_rating} <span className="text-slate-400 font-normal">/ 5</span>
                                            </div>
                                        </div>
                                        <div className="text-xs font-mono text-slate-500">#{r._id.slice(-6)} • {new Date(r.created_at).toLocaleDateString()}</div>
                            ...
                            reports.length === 0 ? <div className="p-8 text-center text-slate-500 text-sm">No open reports.</div> : reports.map(report => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            "w-full text-left p-4 border-b border-slate-100 transition-all hover:bg-slate-100/50 flex flex-col gap-2",
                                            selectedReport?._id === report._id && "bg-violet-50 hover:bg-violet-50 border-l-2 border-l-violet-600 border-b-violet-100"
                                        )}
                                    >
                                        <div className="flex justify-between items-start w-full">
                                            <div className="font-semibold text-sm text-slate-900 truncate pr-2">By: {report.reported_by?.name || 'Unknown'}</div>
                                            <div className="bg-amber-100 text-amber-700 border border-amber-200 rounded-full px-2 py-0.5 text-xs font-medium uppercase shrink-0">{report.reason}</div>
                                        </div>
                                        <div className="text-xs text-slate-500 font-mono">Rpt: #{report._id.slice(-6)} • {new Date(report.createdAt).toLocaleDateString()}</div>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-xs font-medium text-slate-700">Target Visibility:</span>
                                            <div className={cn(
                                                "px-3 py-1.5 rounded-md text-xs font-medium transition border",
                                                r && r.visibility
                                                    ? "bg-emerald-100 text-emerald-700 border-emerald-200"
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-white">
                    <div className="border-b border-slate-200 bg-white py-4 px-6 shrink-0 flex flex-row items-center justify-between">
                        <span className="text-base font-semibold text-slate-800">Detailed Inspector</span>
                        {selectedReview && (
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-sm text-slate-500 mt-1">Written on {new Date(selectedReview.created_at).toLocaleString()}</p>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-3xl font-black text-slate-800 tracking-tight text-amber-500">{selectedReview.average_rating} <span className="text-sm text-slate-400 font-semibold">/ 5</span></div>
                                            </div>
                                        </div>

                                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 relative">
                                            <div className="absolute top-0 left-6 w-8 h-8 -mt-4 bg-slate-100 rounded-full flex items-center justify-center shadow-sm border border-slate-200">
                                            ...
                                            {selectedReview.images && selectedReview.images.length > 0 && (
                                                <div className="flex gap-3 mt-6 pt-6 border-t border-slate-100 overflow-x-auto pb-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            {selectedReview.ai_processed !== false && (
                                                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                                                    <div className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">AI Trust Score</div>
                                            ...
                                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                                                <div className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Community Reports</div>
                                                <div className="text-2xl font-black text-slate-900 tracking-tight mt-auto">{selectedReview.report_count || 0}</div>
                                            </div>
                                        </div>
                                            ...
                                            <div className="space-y-3 mt-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                                {selectedReview.hide_reason && (
                                                    <div className="flex gap-2 text-sm items-center">
                                                        <span className="font-semibold text-slate-900">Hide Reason:</span>
                                                        <Badge variant="danger" className="font-medium shadow-none">{selectedReview.hide_reason}</Badge>
                                                    </div>
                                                )}
                                                {selectedReview.adminActionReason && (
                                                    <div className="flex gap-2 text-sm items-center">
                                                        <span className="font-semibold text-slate-900">Audit Trigger:</span>
                                                ...
                                                {selectedReview.ai_flag_reasons && selectedReview.ai_flag_reasons.length > 0 && (
                                                    <div className="flex gap-2 text-sm items-start">
                                                        <span className="font-semibold text-slate-900 shrink-0 mt-0.5">AI Flags:</span>
                                                        <div className="flex flex-wrap gap-1.5">
                                                            {selectedReview.ai_flag_reasons.map((f, i) => (
                                                                <span key={i} className="bg-violet-100 text-violet-700 border border-violet-200 rounded-full px-2 py-0.5 text-xs font-medium uppercase">{f}</span>
                                                            ))}
                                                        </div>
                                                    </div>
                                                )}
                                ...
                                {selectedReport && (
                                    <div className="mt-8 pt-6 border-t border-slate-200 space-y-6">
                                        ...
                                        <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-4">
                                            <label className="text-slate-800 font-medium text-sm">Administrator Resolution Notes</label>
                                            <textarea
                                                className="w-full rounded-md border border-slate-200 bg-white p-4 text-sm text-slate-700 focus:bg-white focus:ring-2 focus:ring-violet-600 focus:border-transparent min-h-[100px] transition-colors resize-none shadow-sm"
                                                placeholder="Explain your decision to resolve or reject this report... (Required)"
                                                value={adminNotes}
                                            ...
                                            <div className="flex gap-3 pt-2">
                                                ...
                                                >
                                                    <CheckCircle className="h-4 w-4 mr-2" /> Accept & Resolve
                                                </Button>
                                                <Button
                                                    className="flex-1 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 shadow-sm"
                                                    disabled={actionLoading || !adminNotes}
                                                    onClick={() => resolveReport("REJECTED")}
                                                >
                                                    <Ban className="h-4 w-4 mr-2" /> Reject Report
                                                </Button>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="bg-white border-t border-slate-200 p-4 shrink-0 flex flex-col gap-3">
                            {hideReasonOptionsOpen ? (
                                <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                    <label className="text-xs font-semibold text-slate-900 uppercase tracking-widest">Select Hide Reason:</label>
                                    <select
                                        className="w-full text-sm rounded bg-white border border-slate-200 p-2.5 focus:ring-2 focus:ring-violet-600 outline-none"
                                    ...
                                    <div className="flex gap-2">
                                        <Button size="sm" className="flex-1 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 shadow-sm" onClick={() => setHideReasonOptionsOpen(false)}>Cancel</Button>
